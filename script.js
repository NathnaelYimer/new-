document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    try {
      ...
      if (!savedSettings) {
        ...
        if (settings.version !== "1.0" || !settings.productData || !settings.productAssignments) {
          ...
          if (
            typeof settings.productAssignments !== "object" ||
            Object.keys(settings.productAssignments).length === 0
          ) {
    ...
    const priceSettings = JSON.parse(localStorage.getItem("priceSettings")) || {
    ...
    const updatePlanPrice = (plan) => {
      ...
      const months = termElement ? Number.parseInt(termElement.textContent.match(/\d+/)[0]) : 60
    ...
    const productExplanations = productData
      ? Object.fromEntries(Object.entries(productData).map(([name, data]) => [name, data.description]))
      : {}
    ...
    if (priceSettingsModalElement) {
      ...
      priceSettingsModalElement.addEventListener(
        "hidden.bs.modal",
        () => {
          document.querySelectorAll(".modal-backdrop").forEach((backdrop) => backdrop.remove())
          document.body.classList.remove("modal-open")
    ...
    if (productExplanationModalElement) {
      ...
      productExplanationModalElement.addEventListener(
        "hidden.bs.modal",
        () => {
          document.querySelectorAll(".modal-backdrop").forEach((backdrop) => backdrop.remove())
          document.body.classList.remove("modal-open")
    ...
    if (termsModalElement) {
      ...
      termsModalElement.addEventListener(
        "hidden.bs.modal",
        () => {
          document.querySelectorAll(".modal-backdrop").forEach((backdrop) => backdrop.remove())
          document.body.classList.remove("modal-open")
    ...
    // Create a ghost image for touch dragging
    const createDragImage = (element) => {
      ...
      ghostElement.style.top = "-1000px" // Hide it initially
      ...
      ghostElement.style.background = "rgba(255, 255, 255, 0.1)"
      ghostElement.style.backdropFilter = "blur(5px)"

      document.body.appendChild(ghostElement)
      return ghostElement
    }

    // Update ghost image position during touch drag
    const updateDragImagePosition = (x, y) => {
      if (touchDragImage) {
        const rect = touchDragImage.getBoundingClientRect()
        touchDragImage.style.top = `${y - rect.height / 2}px`
        touchDragImage.style.left = `${x - rect.width / 2}px`
      }
    }
    ...
    const getElementFromTouch = (x, y) => {
      // Hide the drag image temporarily to get accurate elementFromPoint
      if (touchDragImage) {
        touchDragImage.style.display = "none"
      }
    ...
    const initializeDragAndDrop = () => {
      ...
      menuTable.removeEventListener("touchstart", handleTouchStart)
      menuTable.removeEventListener("touchmove", handleTouchMove)
      menuTable.removeEventListener("touchend", handleTouchEnd)
      menuTable.removeEventListener("touchcancel", handleTouchCancel)
      ...
      // Add touch event listeners for iPad support
      menuTable.addEventListener("touchstart", handleTouchStart, { passive: false })
      menuTable.addEventListener("touchmove", handleTouchMove, { passive: false })
      menuTable.addEventListener("touchend", handleTouchEnd)
      menuTable.addEventListener("touchcancel", handleTouchCancel)
      ...
      if (trashZone) {
        ...
        // Add touch event listeners for trash zone
        trashZone.removeEventListener("touchmove", handleTouchMove)
        trashZone.removeEventListener("touchend", handleTouchEnd)
        ...
        // Add touch event listeners for trash zone
        trashZone.addEventListener("touchmove", handleTouchMove, { passive: false })
        trashZone.addEventListener("touchend", handleTouchEnd)
      }
    }
    ...
    const handleTouchMove = (e) => {
      if (!isTouchDragging || !draggedItem) {
        // If we're not dragging yet, but have a timeout pending
        if (touchTimeout) {
      ...
      const elementUnderTouch = getElementFromTouch(touch.clientX, touch.clientY)
      ...
      const dropzone = elementUnderTouch?.closest(".kanban-dropzone")
      if (dropzone) {
    ...
    const handleTouchEnd = (e) => {
      ...
      const monthSelectorItem = e.target.closest(".month-selector .dropdown-item")
      if (monthSelectorItem) {
        // Trigger the month selector click handler
        handleMonthSelectorClick(e)
        return
      }
      ...
      // Find the element under the touch point
      const elementUnderTouch = getElementFromTouch(touch.clientX, touch.clientY) || lastTouchTarget
      ...
      // Check if we're over the trash zone
      else if (elementUnderTouch?.closest("#trashZone")) {
        handleTouchTrashDrop()
      }
    ...
    // Handle touch drop on a dropzone
    const handleTouchDrop = (dropzone) => {
    ...
    // Handle touch drop on trash zone
    const handleTouchTrashDrop = () => {
      ...
      settings.productAssignments[plan] = settings.productAssignments[plan].filter((p) => p !== productName)
      draggedItem.remove()
    ...
    const handleDragStart = (e) => {
      ...
      e.dataTransfer.setData(
        "text/plain",
        item.getAttribute("data-product") || item.querySelector(".product-name").textContent,
      )
    ...
    const handleClick = (e) => {
      // Check if the click is on a month selector dropdown item
      const monthSelectorItem = e.target.closest(".month-selector .dropdown-item")
      if (monthSelectorItem) {
        handleMonthSelectorClick(e)
        return
      ...
      if (e.target.closest(".product-name") && !e.target.closest(".remove-product-btn-modal")) {
        ...
        const positionInput = document.querySelector(
          `input[name="pricePosition"][value="${priceSettings[currentPlan].position}"]`,
        )
    ...
    const initializeStaticListeners = () => {
      // We'll handle month selector clicks through delegation in the handleClick and handleTouchEnd functions
      // instead of attaching individual listeners to each dropdown item
    ...
    const populateAddProductDropdown = (plan) => {
      ...
      const existingProducts = Array.from(existingItems).map((item) => item.querySelector(".product-name").textContent)

      const productsToShow = availableProducts.filter((product) => !existingProducts.includes(product))
    ...
    const adjustTableLayout = () => {
      ...
      // Re-initialize event listeners after DOM changes
      initializeDragAndDrop()
      ...
      if (!savedSettings) {
        ...
        try {
          ...
          if (localSettings.version !== "1.0" || !localSettings.productData || !localSettings.productAssignments) {
            ...
            if (
              typeof localSettings.productAssignments !== "object" ||
              Object.keys(localSettings.productAssignments).length === 0
            ) {
